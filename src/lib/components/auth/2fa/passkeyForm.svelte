...
<div class="  mx-auto sm:max-w-[350px] p-5 sm:p-0">
	<div class="grid gap-6 text-center">
		<Icon class="w-36 h-36 m-auto" icon="material-symbols:passkey" />

		<div>
			<h1 class="text-2xl font-bold">{m.lucky_careful_florian_grip()}</h1>
		</div>
		<div class="grid gap-2">
			<Button
				onclick={async () => {
					...
					const response = await fetch(
						mode === 'authenticate' ? '/2fa/passkey' : '/auth/reset-password/2fa/passkey',
						{
							method: 'POST',
							body: JSON.stringify({
			...
			<div>
				...
				<span class="text-sm text-muted-foreground"
					>{m.dark_due_osprey_radiate()} <a class="hover:text-primary underline" href="/auth/2fa/reset"
		...
		<div class="grid gap-2">
			...
			{#if data.user!.registeredSecurityKey}
				<Button variant="outline" href="/auth/2fa/security-key"
					><Icon icon="material-symbols:security-key" />{m.long_ideal_sheep_trip()}</Button
				>
			{/if}
		</div>
	</div>
